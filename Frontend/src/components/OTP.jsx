...
export default function OTPVerification({
  ...
  function submitOtp() {
    ...
    if (otp == bOTP) {
      ...
      toast.success(`Wellcome ${f_name  } ${l_name} `);
    } else {
      ...
      <div className="max-w-md w-full bg-white shadow-lg rounded-2xl p-6">
        ...
        >
          {Array.from({ length }).map((_, idx) => (
            <input
              ...
              className={`w-12 h-12 text-center text-lg rounded-md border focus:outline-none focus:ring-2 focus:ring-green-400 transition-shadow ${
                values[idx] ? "border-green-500 shadow-sm" : "border-gray-200"
              }`}
        ...
        <div className="mt-4 text-center text-sm text-gray-600">
          <p>
            ...
            {disabledResend && (
              <span className="ml-2">(available in {secondsLeft}s)</span>
            )}
          </p>
        </div>
        ...
        <div className="mt-4 text-xs text-gray-400 text-center">
          <p>
            Tip: You can paste the full code (Ctrl/Cmd+V) — the inputs will
            auto-fill.
          </p>
        </div>
      </div>
